 ...
 * @param {String} keyField 当表格可以选择时，所指定的唯一标识字段名
 *   （对应 vxe-grid 的 rowConfig.keyField）
 * */
export const useSelection = (keyField) => {
  ...
  const selectedListIds = computed(() => {
    let ids = [];
    selectedList.value.forEach(item => ids.push(item[keyField])); 
    return ids;
  });
  ...
  const selectedId = computed(() =>
    selectedRow.value ? selectedRow.value[keyField] : null
  );
  // 当前单选是否有选中
