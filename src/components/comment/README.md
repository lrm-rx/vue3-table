...
- **楼层号由后端生成并随评论数据返回**，前端不补排、不取号。组件只透传 `comment.floor`，并在缺失时容错：排序把缺失项排到末尾、渲染不展示「第 n 楼」。
- 楼层是评论的**固定属性**：切到「最热」时列表顺序会变，但每条评论仍显示自己原来的楼层号；删除评论后不重新编号。
- **新发布的一级评论**：乐观插入时**不带 floor**（此时不展示「第 n 楼」），待创建接口返回带真实楼层的记录后，由业务侧调用 `settle(opId, serverItem)` 回填，楼层随即展示。
